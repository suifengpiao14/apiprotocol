...
// 定回调函数指针的类型
type HandlerFn func(interface{}) (interface{}, error)
...
*
*/
func (stream *Stream) Go(arg interface{}) (interface{}, error) {
	//设置为任务链结束
	...
	if stream.firstStream.nextStream != nil {
		return stream.firstStream.nextStream.run(arg)
	} else {
		//流式任务终止
		return nil, errors.New("not found execute node")
	}
}
func (stream *Stream) run(arg interface{}) (interface{}, error) {
	//fmt.Println("run,args=", args)
	//执行本节点函数指针
	result, err := stream.handlerFn(arg)
	//然后调用下一个节点的Run方法
	if stream.nextStream != nil && err == nil {
		return stream.nextStream.run(result)
	} else {
		//任务链终端，流式任务执行完毕
		return result, err
	}
